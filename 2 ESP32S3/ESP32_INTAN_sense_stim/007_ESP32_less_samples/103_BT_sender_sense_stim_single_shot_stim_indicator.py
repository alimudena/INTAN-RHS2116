...
def notification_handler(sender, data):
    ...
    global stim_events_ON, stim_events_OFF, current_stim_state, start_time
    ...
    try:
        ...
        if text == "STIM_ON":
            ...
            stim_events_ON.append((timestamp, 1))
            print(f"⚡ Estimulación ON en t={timestamp:.8f}s")
            return

        elif text == "STIM_OFF":
            timestamp = time.time() - start_time
            current_stim_state = 0
            stim_events_OFF.append((timestamp, 0))
            print(f"⚡ Estimulación OFF en t={timestamp:.8f}s")

            return
    except:
        pass  # No es texto, seguimos con ECG
