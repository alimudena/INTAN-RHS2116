...
def save_to_csv(entry_id):
    if not processed_data:
        messagebox.showinfo("Sin datos", "No hay datos para guardar todavía.")
        return
    ...
    try:
        ...
        with open(filename_csv, mode="w", newline="") as file:
            ...
            writer.writerow(["Indice", "ECG"])
            for i, value in enumerate(processed_data):
                writer.writerow([i, value])

        # --- Crear DataFrame ---
        df = pd.DataFrame({
            "Indice": list(range(len(processed_data))),
            "ECG": processed_data
        })

        # --- Crear gráfica interactiva ---
        fig = px.line(df, x="Indice", y="ECG", title=f"Señal ECG - {experiment_id}")
        fig.update_layout(
            xaxis_title="Índice",
...
def run_gui():
    ...
    ttk.Button(root, text="💾 Guardar datos en CSV y HTML", command=lambda: save_to_csv(entry_id)).pack(pady=5)


    frame_send = ttk.LabelFrame(root, text="Parámetros de estimulación", padding=10)
    frame_send.pack(padx=10, pady=10, fill="x")

    # Campo para ID del experimento
    frame_id = ttk.Frame(frame_send)
    frame_id.pack(fill="x", pady=5)
    ttk.Label(frame_id, text="ID del experimento", width=30).pack(side="left")
