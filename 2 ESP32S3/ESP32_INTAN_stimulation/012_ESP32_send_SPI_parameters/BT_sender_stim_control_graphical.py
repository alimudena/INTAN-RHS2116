...
def notification_handler(sender, data):
    """
    Recibe bytes crudos del ESP32-S3 y los convierte a enteros.
    """
    global received_data
    try:
        values = list(data)  # cada byte → entero 0-255
        received_data.extend(values)
...
def run_async_task(coro):
    """Ejecuta una coroutine en el loop global desde cualquier hilo."""
...
def show_graph():
    ...
    ax.plot(received_data, color="blue", lw=1)
    ax.set_title("Datos SPI recibidos del ESP32S3")
...
if __name__ == "__main__":
    # Inicia el loop asyncio en un hilo independiente para Windows
