    ...
    try:
        ...
        while i <= len(received_raw_buffer) - 3:
            if received_raw_buffer[i] == 0x30:
                ...
                real_value = -19.25 * (combined_value - 512)
                print(high_word, low_word, "->", combined_value)

                # ---------- POSTPROCESADO DE SALTOS BRUSCOS ----------
                # Umbral de diferencia permitido (ajústalo según tu señal)
                THRESHOLD = 20  

                if processed_data:
                    prev_value = processed_data[-1]
                    diff = abs(real_value - prev_value)

                    if diff > THRESHOLD:
                        # Pico detectado → reemplazar con valor promedio estimado
                        # Si hay una siguiente muestra en buffer, usa su valor
                        next_value = None
                        if i + 3 <= len(received_raw_buffer) - 3 and received_raw_buffer[i + 3] == 0x30:
                            high_next = received_raw_buffer[i + 4]
                            low_next = received_raw_buffer[i + 5]
                            combined_next = (high_next << 8) | low_next
                            next_value = -19.25 * (combined_next - 512)

                        if next_value is not None:
                            corrected_value = (prev_value + next_value) / 2
                        else:
                            corrected_value = prev_value  # si no hay siguiente, repite la anterior

                        print(f"⚠️ Pico detectado ({real_value:.2f}) → corregido a {corrected_value:.2f}")
                        real_value = corrected_value
...
def show_realtime_graph():
    ...
    ax.set_title("Señal en tiempo real (últimos 3 s)")
    ax.set_xlabel("Índice relativo")
    ax.set_ylabel("mV")
    ax.set_ylim(-512*19.23, 512*19.23)
    ax.grid(True)

    window_duration = 3  # segundos
    sampling_rate = 10000  # estimación (ajústala según tu frecuencia real de envío)
...
def run_gui():
    ...
    ttk.Button(root, text="📈 Show real-time graph (3s)", command=show_realtime_graph).pack(pady=10)
