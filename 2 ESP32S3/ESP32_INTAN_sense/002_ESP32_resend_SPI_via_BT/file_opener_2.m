...
%% === CREAR LISTA DE ARCHIVOS AUTOMÁTICAMENTE ===
carpeta = 'Medidas';  % 🔹 Cambia esto por la ruta donde están tus archivos
ext = '*.csv';                      % 🔹 Extensión que quieres incluir (puedes cambiar a '*.txt', etc.)
...
%         muestras_valor(end+1,1) = 0.195*(valor-32768)/1000000;  %#ok<SAGROW>
        muestras_valor(end+1,1) = valor;
    end

    % === CREAR VARIABLES DINÁMICAS ===
    nombre_base = erase(rutas(idx), ".csv"); % quitar extensión si la tiene
...
%     fprintf('✅ Generadas variables: %s y %s\n', nombre_tiempo, nombre_valor);
  fprintf('✅ Generada variable: %s\n', nombre_valor);

end

disp('🎯 Todos los archivos procesados correctamente.');
%%
figure
plot(M020_AC_4mVpp_450Hz_muestras_valor)
