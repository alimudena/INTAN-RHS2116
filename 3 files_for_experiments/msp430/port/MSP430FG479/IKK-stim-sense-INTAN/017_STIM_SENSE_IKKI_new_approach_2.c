...
void configure_INTAN(INTAN_config_struct* INTAN_config){
  ...
  */
  ...
  INTAN_config->number_of_pulses = 5;
}
                  ...
                  case STIM:
                    if(timer_flag_stim_rest >= stim_rest_time){
                      ...
                      switch (state_stimulation_ON_OFF) {
                        ...
                        while(number_of_stimulations_done < INTAN_config.number_of_stimulations){
                          number_of_stimulations_done++;
                          ...
                          timer_counter_value = ((uint32_t)stimulation_off_time_milis/10) * ((uint32_t)INTAN_config.MASTER_FREQ / 1000UL);
