...
uint8_t number_of_pulses = 5;
uint8_t number_of_pulses_counter;
...
void receive_parameters_ESP32(INTAN_config_struct* INTAN_config){
    ON_ACK_param();
        ...
        */
        ...
        INTAN_config->MASTER_FREQ = 8000000;
        INTAN_config->resting_time = resting_time_miliseconds*INTAN_config->MASTER_FREQ/(divider_value*200000);
            ...
            if(INTAN_config.single_shot == false){
              if(number_of_stimulations_done <= INTAN_config.number_of_stimulations){ // si el número de estimulaciones hechas es menor al que se quiere hacer
                ...
                switch (state_stimulation) {
                  case STIM:
                    number_of_pulses_counter = number_of_pulses;
                    while(number_of_pulses_counter>0){
                      number_of_pulses_counter--;
                      stim_indicator_ON();
                      ...
                      timer_flag_on_off = 0;
                      timer_counter_value = ((uint32_t)stimulation_off_time_milis) * ((uint32_t)INTAN_config.MASTER_FREQ / 10000UL);
                      // wait for the stimulation time to be over.
                      while(timer_counter_value>0){
                        timer_counter_value--;
                        _no_operation();
