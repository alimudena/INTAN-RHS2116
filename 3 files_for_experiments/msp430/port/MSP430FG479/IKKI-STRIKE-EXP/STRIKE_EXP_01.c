...
*/
uint16_t resting_time_seconds = 0;
uint8_t high_byte_resting_time_seconds = 0;
uint8_t low_byte_resting_time_seconds = 0;
uint16_t stimulation_time_seconds = 0;
uint8_t high_byte_stimulation_time_seconds = 0;
uint8_t low_byte_stimulation_time_seconds = 0;
...
void receive_parameters_ESP32(INTAN_config_struct* INTAN_config){
    ON_ACK_param();
        ...
        */
        ...
        high_byte_resting_time_seconds = UCA0RXBUF;
        
        UCA0TXBUF = 0x20;
        while (!(IFG2 & UCA0RXIFG));              // resting_time_seconds
        low_byte_resting_time_seconds = UCA0RXBUF;
        // ----------------------------------------------------------------------
        // Stimulation time
        // ----------------------------------------------------------------------
        UCA0TXBUF = 0x30;
        while (!(IFG2 & UCA0RXIFG));              // stimulation_time_seconds
        high_byte_stimulation_time_seconds = UCA0RXBUF;

        UCA0TXBUF = 0x30;
        while (!(IFG2 & UCA0RXIFG));              // stimulation_time_seconds
        low_byte_stimulation_time_seconds = UCA0RXBUF;
        ...
        */
        resting_time_seconds = ((uint16_t)high_byte_resting_time_seconds << 8) | low_byte_resting_time_seconds;
        stimulation_time_seconds = ((uint16_t)high_byte_stimulation_time_seconds << 8) | low_byte_stimulation_time_seconds;
        ...
        INTAN_config->MASTER_FREQ = 8000000;
        INTAN_config->resting_time = resting_time_seconds*INTAN_config->MASTER_FREQ/(divider_value*7);
        INTAN_config->stimulation_time = (stimulation_time_seconds*INTAN_config->MASTER_FREQ)/(divider_value*10);
        
        INTAN_config->stimulation_on_time = stimulation_on_time_micro*INTAN_config->MASTER_FREQ*8/(divider_value*1000000*5);
        INTAN_config->stimulation_off_time = stimulation_off_time_milis*INTAN_config->MASTER_FREQ/(10000*divider_value);
