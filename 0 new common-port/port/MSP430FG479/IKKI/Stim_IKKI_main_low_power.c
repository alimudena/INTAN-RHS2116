  ...
  while(1){
    switch (general_state) {
      ...
      case RX_PARAMS_ESP32:{// Estado 2: Recepción de parámetros de estimulación nuevos a través del ESP32
        ...
        // INTAN_config.resting_time = resting_time_minutes*(FREQ_MASTER/divider_value)/2; // s
      ...
      case NEW_PARAM_OFF_WAIT:{ // Estadp 3: Espera a la bajada del pinde comunicaciones
        new_param = new_param_read();
        if(!new_param){
