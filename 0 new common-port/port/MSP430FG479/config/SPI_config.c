...
void SPI_B_setup(SPI_B_config_struct* SPI_config) {
  UCB0CTL0 |= UCSYNC; // Initialyze SPI if not done before

  USCI_SPI_B_pin_setup();
  USCI_B_mode_sel('U');
  SPI_B_mode_config(SPI_config->Master_Slave);

  SPI_B_clk_polarity_phase(SPI_config->inactive_state,
                         SPI_config->data_on_clock_edge);

  SPI_B_char_format(SPI_config->SPI_length,
                  SPI_config->first_Byte_sent); // 8-bit and MSB SPI


  USCI_B_clk_ref(SPI_config->clk_ref_SPI); // CLK reference


  SPI_B_clk_division(SPI_config->clk_div);

  USCI_B_init(); // **Initialize USCI state machine**


  USCI_B_interrupt_enable(
      SPI_config->enable_USCI_interr_rx,
      SPI_config->enable_USCI_interr_tx); // Enable USCI_A0 RX interrupt
}
